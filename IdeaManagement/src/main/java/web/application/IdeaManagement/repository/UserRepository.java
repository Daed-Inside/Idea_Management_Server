...
public interface UserRepository extends JpaRepository<User, String>, JpaSpecificationExecutor<User> {
    ...
    @Query("SELECT dept.department, COUNT(u.userId) \n" +
            "FROM User u \n" +
            "JOIN Department dept ON dept.id = u.departmentId \n" +
            "WHERE (:department IS NOT NULL AND dept.id = :department) or (:department IS NULL)" +
            "GROUP BY dept.id ")
    List<Object[]> countUserByDept(@Param("department") Long department);
